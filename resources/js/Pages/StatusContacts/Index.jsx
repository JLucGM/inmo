...
export default function Index({ auth, statuscontacts, role, permission }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
            roles={role}
            ...
            header={
                <div className='flex justify-between items-center px-6'>
                    <h2 className="capitalize font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">status contacto</h2>
                    <Link href={route('statuscontacts.create')}
                        className="py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                    >
                        Crear status contacto
                    </Link>
                </div>
            }
        >
            <Head className="capitalize" title="status contacto" />
                        ...
                        <div className=" text-gray-900 dark:text-gray-100">
                            ...
                            <div className="relative overflow-x-auto">
                                <table className="w-full border-collapse border text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 hover:border-collapse">
                                    ...
                                    <tbody>
                                        {
                                            statuscontacts?.map((statusContact) => (

                                                <tr key={statusContact.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                    ...
                                                    <td className="border border-slate-200 px-6 py-4">
                                                        <div className='space-x-4'>
                                                            <Link
                                                                className="py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                                                                href={route('statuscontacts.edit', [statusContact])}>
                                                                Editar
                                                            </Link>
                                                            <Link
                                                                ...
                                                                href={route('statuscontacts.destroy', [statusContact])} method='delete' as="button">
                                                                Eliminar
                                                            </Link>
                                                        </div>
                                                    </td>
